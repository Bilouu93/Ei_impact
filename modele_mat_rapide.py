...
class Impact:
    ...
    #Fonction de régression de Kt
        return np.dot(self.Trans, df_ts['Coef reg kt non normalisé'])
    ...
    def Ks(self):
    #Fonction de régression de ps
        return alpha_ps * np.dot(self.Trans, df_ts['Coef reg ps non normalisé'])
    ...
    #Calcule le Wb d'un individu à l'instant t
    ...
    def fonction_impact_gp(self, n):
        ...
        return np.sum(np.vectorize(lambda t: self.fonction_WB_inf(n, t))(T))

    
    def fonction_impact_temps(self, t):
    #Calcule le Wb_collectif à l'instant t
        d_A_n = self.dic_A_n()
        wb_values = np.array([self.fonction_WB_inf(np.sum([d_A_n[t][k][-1][1] + 0.1 for k in range(1, n)]), t) * d_A_n[t][n][-1][1] for n in range(1, self.nb_gp() + 1)])
        return np.sum(wb_values)

    def impact_tot(self):
    #Calcule l'impact brut
        T = np.arange(self.horizon)
        return np.sum(np.vectorize(lambda t: self.fonction_impact_temps(t))(T))
